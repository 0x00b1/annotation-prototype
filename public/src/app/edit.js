define(['../image/layer', '../helper/segment-annotator', '../helper/util'], function (Layer, Annotator, util) {
  function createMainDisplay(params, data, annotator, imageLayer) {
    let imageContainerSpacer = document.getElementById("edit-image-top-menu");
    let annotatorTopMenu = createImageTopMenu(params, data, annotator);
    let annotatorContainer = document.getElementById("edit-image-display");
    ...
    let container = document.getElementById("edit-main-container");
  ...
  function createImageTopMenu(params, data, annotator) {
    ...
    let finerButton = document.createElement("div");
    let boundaryButton = document.createElement("div");
    let coarserButton = document.createElement("div");
    let spacer2 = document.createElement("span");
    let alphaMinusButton = document.createElement("div");
    let imageButton = document.createElement("div");
    let alphaPlusButton = document.createElement("div");
    let container = document.getElementById("edit-image-top-menu");
    ...
    boundaryButton.addEventListener("click", function () {
      ...
      if (boundaryButton.classList.contains("edit-image-top-button-enabled")) annotator.hide("boundary");else annotator.show("boundary");
    ...
    imageButton.addEventListener("click", function () {
      if (imageButton.classList.contains("edit-image-top-button-enabled")) annotator.hide("image");else annotator.show("image");
  ...
  let boundaryFlashTimeoutID = null;

  function boundaryFlash() {
    let boundaryButton = document.getElementById("boundary-button");
  ...
  function createSidebar(params, data, annotator) {
    ...
    let denoiseButton = document.createElement("div");
    let spacer3 = document.createElement("div");
    let superpixelToolButton = document.createElement("div");
    let spacer4 = document.createElement("div");
    let polygonToolButton = document.createElement("div");
    let spacer5 = document.createElement("div");
    let brushToolButton = document.createElement("div");
    let spacer6 = document.createElement("div");
    let manualParagraph = document.createElement("p");
    let spacer7 = document.createElement("div");
  ...
  Annotator.prototype.brush = function (pos, label) {
    ...
    for (let y = -1; y <= 1; y++) {
      for (let x = -1; x <= 1; x++) {
        ...
        let offset = 4 * ((pos[1] + y) * this.layers.visualization.canvas.width + (pos[0] + x));
  ...
  function render(data, params) {
    ...
    var image_id = parseInt(params.id, 10);

    var container = document.getElementById("edit-top-menu-block");
    ...
    if (image_id > 0) {
      previous.href = util.makeQueryParams(params, { id: image_id - 1 });
    } else {
      previous.href = "";
    }

    index.href = util.makeQueryParams({ view: "index" });

    if (image_id < data.imageURLs.length - 1) {
      next.href = util.makeQueryParams(params, { id: image_id + 1 });
    } else {
      next.href = "";
    }

    container.appendChild(document.createTextNode(data.imageURLs[params.id]));

    navigation.appendChild(container);

    var application = document.getElementById("application");

    application.appendChild(createMainDisplay(params, data, annotator, imageLayer));
  }
