define(['../image/layer', '../image/segmentation', '../image/morph'], function (Layer, segmentation, morph) {
    ...
    Annotator.prototype.getUniqueLabels = function () {
        ...
        for (let i = 0; i < data.length; i += 4) {
            let label = _getEncodedLabel(data, i);
    ...
    Annotator.prototype.fill = function (targetLabel) {
        let pixels = [];
        let annotationData = this.layers.annotation.imageData.data;

        for (let i = 0; i < annotationData.length; i += 4) {
            let label = _getEncodedLabel(annotationData, i);
    ...
    Annotator.prototype.export = function () {
        ...
        let data = this.layers.annotation.canvas.toDataURL();
    ...
    Annotator.prototype.highlightLabel = function (label) {
        ...
        for (let i = 0; i < annotationData.length; i += 4) {
            let currentLabel = _getEncodedLabel(annotationData, i);
    ...
    Annotator.prototype.denoise = function () {
        let indexImage = morph.decodeIndexImage(this.layers.annotation.imageData);

        let result = morph.maxFilter(indexImage);

        let pixels = new Int32Array(result.data.length);
    ...
    Annotator.prototype._createLayers = function (options) {
        ...
        this.container = document.getElementById("segment-annotator-outer-container");

        this.container.classList.add("segment-annotator-outer-container");

        this.innerContainer = document.getElementById("segment-annotator-inner-container");
        ...
        for (let key in this.layers) {
            let canvas = this.layers[key].canvas;
    ...
    Annotator.prototype._initializeEvents = function () {
        let canvas = this.layers.annotation.canvas;
        let mousestate = { down: false, button: 0 };
        let annotator = this;
        ...
        function update(e) {
            let offset = annotator._getClickOffset(e);
            let superpixelData = annotator.layers.superpixel.imageData.data;
            let annotationData = annotator.layers.annotation.imageData.data;
            let superpixelIndex = _getEncodedLabel(superpixelData, offset);
            let pixels = annotator.pixelIndex[superpixelIndex];
            let existingLabel = _getEncodedLabel(annotationData, offset);
            ...
            if (mousestate.down) {
                if (mousestate.button === 2 && typeof annotator.onrightclick === "function") {
                    ...
                    if (annotator.mode === "brush" && e.button === 0) {
                        annotator.brush(annotator._getClickPos(e), annotator.currentLabel);
                    }

                    if (e.button === 0 && annotator.mode === "polygon") {
                        annotator._addPolygonPoint(e);
        ...
        window.addEventListener("keydown", function (event) {
            ...
            // FIXME: We should check whether the key is in the list of classes rather than checking whether an element exists.
            const identifier = document.getElementById("label-" + String.fromCharCode(key - 1) + "-button");

            // FIXME: The `click` event should be moved into the edit module.
            if (identifier) {
                identifier.click();
            }
        });
    ...
    Annotator.prototype._updateBoundaryLayer = function () {
        let boundaryLayer = this.layers.boundary;
    ...
    Annotator.prototype._initializeAnnotationLayer = function () {
        let layer = this.layers.annotation;
    ...
    Annotator.prototype._initializeVisualizationLayer = function () {
        let layer = this.layers.visualization;
        layer.resize(this.width, this.height);
        let initialColor = this.colormap[this.defaultLabel].concat([this.visualizationAlpha]);
    ...
    Annotator.prototype._createPixelIndex = function (numSegments) {
        let pixelIndex = new Array(numSegments),
        ...
        for (i = 0; i < data.length; i += 4) {
            let index = data[i] | data[i + 1] << 8 | data[i + 2] << 16;
    ...
    Annotator.prototype._getClickPos = function (event) {
        var container = this.container,
            ...
            x = Math.round((event.pageX - offsetLeft + container.scrollLeft) * (container.offsetWidth / container.scrollWidth)),
    ...
    Annotator.prototype._addPolygonPoint = function (event) {
        ...
        //get canvas.
        let canvas = annotator.layers.annotation.canvas;
    ...
    Annotator.prototype._emptyPolygonPoints = function () {
        let annotator = this;
        let ctx = annotator.layers.annotation.canvas.getContext('2d');
    ...
    Annotator.prototype._updateHighlight = function (pixels) {
        let visualizationData = this.layers.visualization.imageData.data,
    ...
    Annotator.prototype._getDifferentialUpdates = function (pixels, labels) {
        ...
        for (let i = 0; i < pixels.length; ++i) {
            let label = _getEncodedLabel(annotationData, pixels[i]);
            if (label !== labels[i]) {
